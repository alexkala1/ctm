...
export default defineEventHandler(async event => {
  try {
    ...
    if (!['tournament', 'competitor'].includes(type)) {
      throw createError({
        statusCode: 400,
        statusMessage: 'Invalid template type. Must be "tournament" or "competitor"',
      });
    }
    ...
    const templateData = generateImportTemplate(type as 'tournament' | 'competitor');

    if (format === 'csv') {
      // Generate CSV
      const csvHeaders = Object.keys(templateData[0]);
      const csvRows = templateData.map(row => csvHeaders.map(header => `"${row[header] || ''}"`).join(','));
