...
export default defineEventHandler(async event => {
  try {
    ...
    if (duplicateTournament) {
      throw createError({
        statusCode: 400,
        statusMessage: 'A tournament with this name already exists. Please rename the tournament before restoring.',
      });
    }
    ...
    if (process.env.NODE_ENV === 'development') console.error('Restore tournament error:', error);
    throw createError({
      statusCode: 500,
