...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (
      ...
      !existingCompetitor.tournament.categories.includes(validatedData.category)
    ) {
      throw createError({
        statusCode: 400,
    ...
    if (validatedData.ratedPlayerLinks !== undefined)
      updateData.ratedPlayerLinks = validatedData.ratedPlayerLinks
    if (validatedData.tournamentDocumentUrl !== undefined)
      updateData.tournamentDocumentUrl =
        validatedData.tournamentDocumentUrl ?? null
    ...
    if (validatedData.adminNotes !== undefined)
      updateData.adminNotes = validatedData.adminNotes ?? null
