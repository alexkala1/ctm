...
import type { Tournament, Competitor, ImportResult } from '../../types/tournament';
...
export async function exportToCSV(
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') console.error('CSV export failed:', error);
    return {
      success: false,
...
export async function exportToExcel(
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') console.error('Excel export failed:', error);
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Export failed',
    };
...
export async function importFromCSV(file: File, schema: ZodSchema): Promise<ImportResult> {
...
export async function importFromExcel(file: File, schema: ZodSchema): Promise<ImportResult> {
  try {
...
export function createTournamentExportData(tournaments: Tournament[]): Record<string, unknown>[] {
  return tournaments.map(tournament => ({
    ...
    'Registration Start': new Date(tournament.tournamentRegistrationStart).toLocaleDateString(),
    'Registration End': new Date(tournament.tournamentRegistrationEnd).toLocaleDateString(),
...
export function createCompetitorExportData(competitors: Competitor[]): Record<string, unknown>[] {
...
export function createTournamentImportSchema() {
  return {
    type: 'object',
    required: ['name', 'tournamentStart', 'tournamentEnd', 'tournamentRegistrationStart', 'tournamentRegistrationEnd'],
    properties: {
...
export function generateImportTemplate(type: 'tournament' | 'competitor'): Record<string, unknown>[] {
  if (type === 'tournament') {
    return [
      {
