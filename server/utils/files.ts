...
export async function uploadFile(file: File, bucket: string, path: string, userId: string): Promise<FileUploadResult> {
  try {
    // Validate file
    ...
    if (process.env.NODE_ENV === 'development') console.error('File upload failed:', error);
    return {
      success: false,
...
export async function deleteFile(bucket: string, path: string): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    if (process.env.NODE_ENV === 'development') console.error('File deletion failed:', error);
    return {
      success: false,
...
export async function getSignedUrl(bucket: string, path: string, expiresIn: number = 3600): Promise<string | null> {
  try {
    ...
    if (error) {
      if (process.env.NODE_ENV === 'development') console.error('Failed to create signed URL:', error);
      return null;
    }

    return data.signedUrl;
  } catch (error) {
    if (process.env.NODE_ENV === 'development') console.error('Signed URL creation failed:', error);
    return null;
  }
}
...
export async function uploadProclamation(file: File, tournamentId: string, userId: string): Promise<FileUploadResult> {
  const path = `tournaments/${tournamentId}/proclamations`;
  return uploadFile(file, 'tournament-documents', path, userId);
}

export async function uploadChessResults(file: File, tournamentId: string, userId: string): Promise<FileUploadResult> {
...
export async function scanFileForViruses(file: File): Promise<{ clean: boolean; error?: string }> {
...
export async function cleanupOrphanedFiles(): Promise<{
  ...
  try {
    ...
    for (const file of files) {
      ...
      const { error: deleteError } = await supabase.storage.from('tournament-documents').remove([file.name]);
