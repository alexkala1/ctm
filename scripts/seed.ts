...
async function main() {
  ...
  if (process.env.NODE_ENV === 'development')
    console.log('✅ Created admin users and regular user')
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const tournamentEnd = new Date(
      tournamentStart.getTime() + 3 * 24 * 60 * 60 * 1000
    ) // 3 days later
    const registrationStart = faker.date.past()
    const registrationEnd = new Date(
      tournamentStart.getTime() - 7 * 24 * 60 * 60 * 1000
  ...
  for (const tournament of tournaments) {
    ...
    if (process.env.NODE_ENV === 'development')
      console.log(
        `✅ Created ${competitorCount} competitors for tournament: ${tournament.name}`
      )
  }

  // Note: Audit logs will be created automatically when using the application
  if (process.env.NODE_ENV === 'development')
    console.log('✅ Skipping audit logs (will be created during app usage)')

  if (process.env.NODE_ENV === 'development')
    console.log('🎉 Database seeding completed!')
  if (process.env.NODE_ENV === 'development') console.log('\n📊 Summary:')
  if (process.env.NODE_ENV === 'development')
    console.log(`- Users: 3 (1 Super Admin, 1 Admin, 1 Regular User)`)
  if (process.env.NODE_ENV === 'development')
    console.log(`- Tournaments: ${tournaments.length}`)
  if (process.env.NODE_ENV === 'development')
    console.log(`- Total Competitors: ${await prisma.competitor.count()}`)
  if (process.env.NODE_ENV === 'development')
    console.log(`- Audit Logs: ${await prisma.auditLog.count()}`)
  if (process.env.NODE_ENV === 'development')
    console.log('\n🔑 Login Credentials:')
  if (process.env.NODE_ENV === 'development')
    console.log('Super Admin: admin@chess-tournament.com')
  if (process.env.NODE_ENV === 'development')
    console.log('Admin: tournament@chess-tournament.com')
  if (process.env.NODE_ENV === 'development')
    console.log('Regular User: user@chess-tournament.com')
}
