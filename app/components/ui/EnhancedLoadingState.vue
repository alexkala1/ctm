<template>
  <div class="enhanced-loading-state">
    <div class="text-center py-12 px-4">
      <!-- Animated Icon -->
      <div
        class="mx-auto flex items-center justify-center h-24 w-24 rounded-full bg-primary-50 dark:bg-primary-900/20 mb-6"
      >
        <div class="relative">
          <Icon :name="icon" class="h-12 w-12 text-primary-600 dark:text-primary-400 animate-spin" />
          <div
            v-if="showPulse"
      ...
      <div v-if="steps && steps.length > 0" class="max-w-md mx-auto">
        <div class="space-y-3">
          ...
          >
            ...
            >
              <Icon v-if="index < currentStep" name="i-heroicons-check" class="w-4 h-4" />
              <Icon v-else-if="index === currentStep" name="i-heroicons-arrow-path" class="w-4 h-4 animate-spin" />
