  ...
  <div class="max-w-2xl mx-auto">
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg">
      <div class="px-4 py-5 sm:p-6">
        ...
        <VeeForm :validation-schema="schema" :initial-values="initialValues" class="space-y-6" @submit="handleSubmit">
          <!-- Personal Information -->
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <VeeField v-slot="{ field, errorMessage }" name="firstName">
              <UFormGroup label="First Name" :error="errorMessage" required>
                <UInput v-bind="field" placeholder="Enter first name" :disabled="isSubmitting" />
              </UFormGroup>
            </VeeField>

            <VeeField v-slot="{ field, errorMessage }" name="lastName">
              <UFormGroup label="Last Name" :error="errorMessage" required>
                <UInput v-bind="field" placeholder="Enter last name" :disabled="isSubmitting" />
              </UFormGroup>
            </VeeField>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <VeeField v-slot="{ field, errorMessage }" name="gender">
              <UFormGroup label="Gender" :error="errorMessage" required>
                <USelect v-bind="field" :items="genderOptions" placeholder="Select gender" :disabled="isSubmitting" />
              </UFormGroup>
            </VeeField>
          ...
          <div v-if="tournament?.hasTeams" class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <UFormGroup label="Tournament Document" :error="errorMessage">
              ...
              >
                <div class="space-y-1 text-center">
                  <Icon name="i-heroicons-document" class="mx-auto h-12 w-12 text-gray-400 dark:text-gray-500" />
                  ...
                  <p class="text-xs text-gray-500 dark:text-gray-400">PDF, DOC, DOCX, JPG, PNG up to 5MB</p>
                </div>
              </div>
          ...
          <VeeField v-if="isAdmin" v-slot="{ field, errorMessage }" name="adminNotes">
            <UFormGroup label="Admin Notes" :error="errorMessage">
              <UTextarea v-bind="field" placeholder="Add any admin notes..." :disabled="isSubmitting" rows="3" />
            </UFormGroup>
          </VeeField>

          <!-- Status (Admin only) -->
          <VeeField v-if="isAdmin" v-slot="{ field, errorMessage }" name="playerAcceptanceStatus">
            <UFormGroup label="Status" :error="errorMessage">
              <USelect v-bind="field" :items="statusOptions" placeholder="Select status" :disabled="isSubmitting" />
            </UFormGroup>
          </VeeField>

          <!-- Actions -->
          <div class="flex justify-end space-x-4 pt-6 border-t border-gray-200 dark:border-gray-700">
            <UButton type="button" variant="soft" :disabled="isSubmitting" @click="$emit('cancel')"> Cancel </UButton>
...
const teamOptions = computed(() => {
  ...
  const existingTeams = tournament?.competitors?.map(c => c.team).filter(team => team && team.trim() !== '') || [];

  const uniqueTeams = [...new Set(existingTeams)];

  return [{ label: 'No Team', value: '' }, ...uniqueTeams.map(team => ({ label: team, value: team }))];
});
...
const schema = object({
  ...
  playerAcceptanceStatus: string().oneOf(['PENDING', 'APPROVED', 'REJECTED'], 'Invalid status').nullable(),
});
