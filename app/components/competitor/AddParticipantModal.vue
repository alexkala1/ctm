...
import type { TournamentApiResponse, CompetitorApiResponse, Gender, PlayerAcceptanceStatus } from '~/types/tournament';
...
const emit = defineEmits<{
  participantAdded: [participant: CompetitorApiResponse];
  close: [];
}>();
...
const validationSchema = z.object({
  ...
  team: z.string().max(100, 'Team name must be less than 100 characters').optional().or(z.literal('')),
  ratedPlayerLinks: z
    .string()
    .optional()
    .refine(val => {
...
const handleSubmit = async (_event: Event) => {
  try {
    ...
    if (response.success) {
      emit('participantAdded', response.data);
      emit('close');
    } else {
