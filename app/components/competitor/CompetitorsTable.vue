...
const emit = defineEmits<{
  ...
  delete: [id: string]
  'view-details': [competitor: CompetitorWithDetails]
  'import-participants': []
  'export-participants': []
}>()
...
const filteredCompetitors = computed(() => {
  const filtered = props.competitors.filter((competitor) => {
    ...
    const matchesCategory =
      ...
      competitor.category === filters.value.category
    const matchesGender =
      !filters.value.gender ||
      filters.value.gender === 'all' ||
      competitor.gender === filters.value.gender
    const matchesStatus =
...
const confirmDelete = async () => {
  ...
  try {
    const response = await $fetch<{ success: boolean }>(
      `/api/competitors/${competitorToDelete.value.id}`,
      {
        method: 'DELETE',
      }
    )
