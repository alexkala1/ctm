<template>
  <div class="p-6 max-h-[70vh] overflow-y-auto">
    <UForm :state="formData" :schema="validationSchema" class="space-y-8" @submit="handleSubmit">
      ...
      <div class="space-y-6">
        <div v-for="(_category, index) in formData.categories" :key="index" class="flex items-center gap-3">
          <UInput
          ...
          <div class="flex items-start space-x-3">
            ...
            <div>
              ...
              <p class="text-sm text-blue-700 dark:text-blue-300 mt-1">
                When teams are enabled, competitors can join existing teams or create new ones during registration.
                Teams will be automatically created as competitors register with team names.
              </p>
            </div>
          </div>
        </div>
...
const emit = defineEmits<{
  tournamentCreated: [tournament: TournamentApiResponse];
  close: [];
}>();
...
const _formatDateRange = (range: { start: CalendarDate | null; end: CalendarDate | null } | null) => {
...
const handleSubmit = async (event: FormSubmitEvent<z.infer<typeof validationSchema>>) => {
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') console.error('Error creating tournament:', error);
