    ...
    if (tournament) {
      formData.value = {
        ...
        tournamentEnd: tournament.tournamentEnd
          ? new Date(tournament.tournamentEnd).toISOString().split('T')[0]
          : '',
        tournamentRegistrationStart: tournament.tournamentRegistrationStart
          ? new Date(tournament.tournamentRegistrationStart)
              .toISOString()
              .split('T')[0]
          : '',
        tournamentRegistrationEnd: tournament.tournamentRegistrationEnd
          ? new Date(tournament.tournamentRegistrationEnd)
              .toISOString()
              .split('T')[0]
          : '',
        categories:
...
watch(
  ...
  (newRange: DateRange) => {
    if (newRange.start && newRange.end) {
      formData.value.tournamentStart = newRange.start
        .toDate(getLocalTimeZone())
        .toISOString()
        .split('T')[0]
      formData.value.tournamentEnd = newRange.end
        .toDate(getLocalTimeZone())
        .toISOString()
        .split('T')[0]
    }
  ...
  (newRange: DateRange) => {
    if (newRange.start && newRange.end) {
      formData.value.tournamentRegistrationStart = newRange.start
        ...
        .toISOString()
        .split('T')[0]
      formData.value.tournamentRegistrationEnd = newRange.end
        .toDate(getLocalTimeZone())
        .toISOString()
        .split('T')[0]
    }
...
const handleSubmit = async (
  ...
  try {
    ...
    if (response.success) {
      emit('tournament-updated', response.data)
      emit('close')
    } else {
